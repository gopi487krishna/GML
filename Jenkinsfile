    ...
    stages {
        ...
        stage('Deploy'){
            steps {
                ...
                sh 'cd gml-docs && sudo git add .  && sudo git commit -m "Updated Documentation" && sudo git remote rm origin && sudo git remote add origin git@github.com:gopi487krishna/gml-docs.git && sudo git push origin master'
            }
        }
    }
    post {
